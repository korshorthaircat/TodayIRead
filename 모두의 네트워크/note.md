...
* 컴퓨터와 DNS 서버간에 일어나는 IP 주소 정보교환
* 요청받은 DNS 서버가 해당 도메인 이름의 IP 주소를 모르는 경우에는 다른 DNS tjqjdp wlfdmlgka
* DNS 서버는 전 세계에 흩어져 있으므로 연계하며 동작함
...
* POP3
  * 메일 수신시 사용되는 프로토콜
  * 포트번호: 110
* 메일 서버간에도 SMTP를 이용해 메일을 전송한다.
* 메일 서버에는 메일박스가 있어 여기에 메일이 보관된다.

## ping 명령
* 목적지 컴퓨터와 통신을 확인시 `ping` 명령을 이용함
* ICMP(Internet Control Message Protocol)을 이용하여 목적지 컴퓨터에 ICMP 패킷을 전송하고 패킷에 대한 응답이 제대로 오는지 확인하는 명령어
* 명령 정상 실행시 네트워크 연결이 정상이라고 판단할 수 있음 -> 문제 확인을 위해 자주 사용함
```
ping www.google.co.kr //ping 목적지호스트이름
```
- - -
# Ch.8 네트워크의 전체 흐름 살펴보기
- - -
# Ch.9 무선 랜 이해하기

## 무선 랜
* 랜 케이블을 사용하지 않고 눈에 보이지 않는 전파를 이용해 무선으로 컴퓨터를 서로 연결함
* 장점
* 단점
  * 유선 랜에 비해 통신 내용이 해킹될 위험이 높아 보안에 신경써야 함 -> 암호화나 인증 설정 필요 
### 무선 랜의 구성
* 무선 랜에서는 무선 액세스포인트와 무선 클라이언트가 서로 통신함
  * 무선 액세스 포인트(wireless access point)
    * 무선 공유기 등이 무선 액세스 포인트 기능을 포함함
    * 비컨(beacon)이라는 자기를 알리는 신호를 네트워크에 있는 모든 기기에 주기적으로 전송하고, 무선 클라이언트는 이 신호를 잡아 연결하는 것임
    * 비컨을 받은 무선 클라이언트는 자신의 SSID와 같은지 무선액세스포인트에 문의함
    * 인증 방식이 올바른지 확인 되면 무선 클라이언트는 무선액세스포인트에 연결을 요청하고, 무선액세스포인트로부터 승인을 받으면 연결하여 통신이 가능함
  * 무선 클라이언트(컴퓨터나 스마트폰 등)
    * 최근 나오는 컴퓨터는 대부분 `무선 랜 칩`을 내장하고 있어 무선 액세스 포인트와 통신 가능
    * `무선 랜 어댑터`: `USB 메모리 방식 어댑터`와 `컴퓨터 카드 방식 어댑터`가 있음
### 무선 랜을 연결하는 방식
* `인프라스트럭처 방식`
  * 무선 액세스 포인트(무선 공유기)를 통해 통신하는 방식
  * 일반적으로 인프라스트럭처 방식을 많이 사용함
* `애드혹 방식`
  * 무선 클라이언트끼리 직접 통신하는 방식
### 무선 랜 규격
* `IEEE802.11` 규격을 준수함: 미국 기술화 표준 단체인 IEEE에서 승인한 무선 랜의 표준화 기술을 준수함

## SSID(Service Set IDentifier, 액세스 포인트 공유 이름)
